...
void declareTypesSBA(py::module& m) {
  ...
  templatedBaseBinaryEdge<2, Vector2D, VertexSBAPointXYZ, VertexCam>(
      m, "_2_Vector2D_VertexSBAPointXYZ_VertexCam");
  ...
  // stereo projection
  // first two args are the measurement type, second two the connection classes
  templatedBaseBinaryEdge<3, Vector3D, VertexSBAPointXYZ, VertexCam>(
      m, "_3_Vector3D_VertexSBAPointXYZ_VertexCam");
  ...
  py::class_<EdgeProjectP2MC_Intrinsics, BaseMultiEdge<2, Vector2D>>(
      m, "EdgeProjectP2MC_Intrinsics")
      .def(py::init<>())
      .def("compute_error",
           &EdgeProjectP2MC_Intrinsics::computeError)  // () -> void
  ...
  templatedBaseEdge<6, VertexCam>(m, "_6_VertexCam");
  templatedBaseMultiEdge<6, VertexCam>(m, "_6_VertexCam");
  py::class_<EdgeSBALinearMotion, BaseMultiEdge<6, VertexCam>>(
      m, "EdgeSBALinearMotion")
      .def(py::init<>())
      .def("compute_error", &EdgeSBALinearMotion::computeError);
}
